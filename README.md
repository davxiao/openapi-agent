...
For testing, in a seperate shell, run 

```
curl -X GET "http://localhost:8081/current_time?city=nyc" -H "Authorization: Bearer <replace-with-oauth2-access-token>"
``` 

to verify the server works properly.

In addition, try 

```
curl -X GET "http://localhost:8081/current_time?city=nyc"
```

Without authorization header, it should return `"Not authenticated"` as expected.
...
**⚠️ Make sure to update the relevant Okta values in `adk_agent/.env`**
...
In adk web input, enter "what can you do" and it should trigger Okta OAuth flow. If encounter Okta error, please check the Okta configuration and ensure `http://localhost:8085/dev-ui/` is added in Okta as redirect URI.
